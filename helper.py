...
def most_common_words(selected_user,df):
    f = open('E:\Aditya\whatsappChatAnalyzer\stop_hinglish.txt','r')
    stop_words = f.read()
...
def emoji_stats(selected_user,df):
    ...
    emojis_list = []
    for message in df['message']:
        emojis_list.extend([char for char in message if char in emoji.EMOJI_DATA])
    emoji_df = pd.DataFrame(Counter(emojis_list).most_common(),columns=['Emoji', 'Count'])
    return emoji_df
